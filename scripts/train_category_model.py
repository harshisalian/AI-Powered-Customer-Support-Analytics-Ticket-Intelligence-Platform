...
MODEL_OUTPUT_PATH = Path("models/category_model.pkl")
REPORT_OUTPUT_PATH = Path("reports/category_model_report.txt")
RANDOM_STATE = 42
...
def load_training_data(dataset_path: Path = DATASET_PATH) -> tuple[pd.Series, pd.Series]:
    ...
    return dataset["clean_text"], dataset["category"]
...
def write_report(best_model_name: str, metrics: dict[str, dict[str, float | str]]) -> None:
    ...
    lines = [
        "Category Classification Model Report",
        "====================================",
        "",
